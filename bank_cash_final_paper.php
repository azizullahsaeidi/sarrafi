			...
			<table>
				<tr>
					...
					<th>
						...
						<input type="hidden" name='giver_type' id='giver_p_type' value="bank_r" style="margin-bottom: 0px;height: 26px">
					</th>
...
//$query=mysql_query("select * from bank_cash where cash_serial_no=$s_n and cash_cur_id=$cur and cash_date like '$year-%'",$con);
$query=mysql_query("select * from bank where bank_serial_no=$s_n and bank_cur_id=$cur and bank_type='bank_a' and bank_date like '$year-%'",$con);
$giver_query=mysql_query("select g_person from giver_person where g_serial=$s_n and  g_type='bank_r' and g_year = $year",$con);
if(mysql_num_rows($query)>0){
$row=mysql_fetch_assoc($query);extract($row);
//$name=explode("/", $box_reciever_name);
$number= en2f_number(number_format($bank_amount));
$box_serial=0;
$cash_emp_date=en2f_number($bank_cash_issue_date);
$box_9=0;
$box_11=0;
if ($s_n<=9) {
	$box_9='00';
...
?>
	<!-- Begining of Content -->
		...
		<a href="bank_cash_edit.php?cur=<?php echo $cur;?>&cash_id=<?php echo $ban_id;?>" >
		...
		<a href="../controller/bank_query.php?cur=<?php echo $cur;?>&s_n=<?php echo $s_n;?>&cash_id_for_delete=<?php echo $ban_id;?>" onClick="return confirm('آیا حاضر به حذف این برگ هستید؟')">
			...
			<div class="span10 add_table"  id='payment_paper'>
				<table class="table_setting"   style="width:100%">
					...
					<tr> 
					 	...
					 	<th colspan='6'><h3 style='height:35px;margin:5px;font-size: 22px'>دفتر نمایندگی کمیته امداد امام خمینی (ره) در افغانستان</h3></th>
					 	<th colspan='1'></th>
					</tr>
					<tr> 
					...
					<tr> 
					 	...
					 	<th colspan='4' >تاریخ: 
					 	<?php 
					 	$insetDate=explode('-', $row['bank_date']);
					 	$datas=$insetDate[0].'/'.$insetDate[1].'/'.$insetDate[2];
					 	echo en2f_number($datas); 
					 	?></th>
				...
				<table class="table_setting" border="1" style="width:100%">
					<tr> 
					 	...
					 	<td colspan='5' style='width:75%;text-align:right'> <b>به حروف: </b> <?php echo convert_number_to_words($bank_amount); ?>« <?php echo get_currency_id('cur_name'); ?> »</td>
					</tr>
					<tr> 
					 	<td colspan='8' style='width:100%;text-align:right;padding-right:3px;'><input type='checkbox'  checked > 
					 		<b style="margin-left:10px;">واریز به حساب بانک: </b> <l>« </l> <?php $b_name=explode("/", $bank_check_number); echo $b_name[0]; ?><l> »</l>
					 		<b style="margin-left:3px;margin-right:10px;">شماره حساب: </b> <l>« </l> <?php echo $bank_account_number; ?><l> »</l>
					 		<b style="margin-left:3px;margin-right:10px;">طی رسید شماره: </b> <l>« </l> <?php echo en2f_number($bank_cash_no); ?><l> »</l>
					 		<b style="margin-left:3px;margin-right:10px;">مورخ: </b> <l>« </l> <?php echo $cash_emp_date; ?><l> »</l>
					 	</td>
				...
				<table class="table_setting"  style="width:100%">
					<tr style="height:40px;"> 
					 	<td colspan='8' style='width:100%;text-align:right;padding-right:30px;'><b>مورد دریافت: </b> 
					 		...
					 		وجوهات امانی:
					 		<?php if ($bank_reciever_name=='wojohat') {
					 			echo "<input type='checkbox' checked style='margin-left:20px;'  >";
					 		}else{
					 			echo "<input type='checkbox' style='margin-left:20px;'  >";
					 		}?> 
					 		فروش:
					 		<?php if ($bank_reciever_name=='sale') {
					 			echo "<input type='checkbox' checked style='margin-left:20px;'  >";
					 		}else{
					 			echo "<input type='checkbox' style='margin-left:20px;'  >";
					 		}?> 
					 		وصول مطالبات:
					 		<?php if ($bank_reciever_name=='study') {
					 			echo "<input type='checkbox' checked style='margin-left:20px;'  >";
					 		}else{
					 			echo "<input type='checkbox' style='margin-left:20px;'  >";
					 		}?> 
					 		وام:
					 		<?php if ($bank_reciever_name=='borrow') {
					 			echo "<input type='checkbox' checked style='margin-left:20px;'  >";
					 		}else{
					 			echo "<input type='checkbox' style='margin-left:20px;'  >";
					 		}?> 
					 		ایتام:
					 		<?php if ($bank_reciever_name=='orphan') {
					 			echo "<input type='checkbox' checked style='margin-left:20px;'  >";
					 		}else{
					 			echo "<input type='checkbox' style='margin-left:20px;'  >";
					 		}?> 
					 		اعتبارات:
					 		<?php if ($bank_reciever_name=='other') {
					 			echo "<input type='checkbox' checked style='margin-left:20px;'  >";
					 		}else{
					 			echo "<input type='checkbox' style='margin-left:20px;'  >";
					 		}?>
					...
					<tr> 
					 	<td colspan='8' style='width:75%;text-align:right;padding-right:30px;'> <b>توضیحات: </b> <?php echo $bank_reason; ?></td>
					</tr>
					<tr style="height:40px;"> 
				...
				<form action="../controller/bank_query.php" method="post" id="payment_paper">
					...
					<tr>
						...
						<td class='submit_reset my_print' style='text-align:left;'>
							<input type="hidden" name="bank_cash_currency_id"  value="<?php echo get_currency_id('cur_id'); ?>"  >
